...
void doRingFilterOMP(float*** polar_image, int pol_height, int pol_width, float threshold, int m_rad, int m_azi, int ring_width, ImageFilterClass* filter_machine, int verbose){
	float* image_block = (float *) calloc(pol_height*pol_width, sizeof(float ));
	float** filtered_image = (float **) calloc(pol_height, sizeof(float*));
	filtered_image[0] = image_block;
	for(int i=1; i<pol_height; i++){
	...
	if(verbose == 1) printf("Performing Radial Filter on polar image... \n");
	//clock_t start_median = clock();
	double start_median = omp_get_wtime();
...
//	filter_machine->doMedianFilter1D(&filtered_image, polar_image, 0, 0, pol_height-1, pol_width-1, 'x', (ring_width - 1)/2, ring_width, pol_width, pol_height);
	...
	{
		#pragma omp section
		filter_machine->doMedianFilterFast1DOMP(&filtered_image, polar_image, 0, 0, pol_height-1, pol_width/3 -1, 'x', m_rad/3, ring_width, pol_width, pol_height);

		#pragma omp section
		filter_machine->doMedianFilterFast1DOMP(&filtered_image, polar_image, 0, pol_width/3, pol_height-1, 2*pol_width/3 -1, 'x', 2*m_rad/3, ring_width, pol_width, pol_height);

		#pragma omp section
		filter_machine->doMedianFilterFast1DOMP(&filtered_image, polar_image, 0, 2*pol_width/3, pol_height-1, pol_width-1, 'x', m_rad, ring_width, pol_width, pol_height);
	}

	//clock_t end_median = clock();
	double end_median = omp_get_wtime();
	//if(verbose == 1) printf("Time for median filter: %f sec \n", (float(end_median - start_median)/CLOCKS_PER_SEC));
	if(verbose == 1) printf("Time for median filter: %f sec \n", float(end_median - start_median));
	...
	if(verbose == 1) printf("Performing Azimuthal mean filter... \n");
	//clock_t start_mean = clock();
	double start_mean = omp_get_wtime();

	#pragma omp parallel sections num_threads(3)
	{
		#pragma omp section
		filter_machine->doMeanFilterFast1DOMP(&filtered_image, polar_image, 0, 0, pol_height-1, pol_width/3-1, 'y', m_azi/3, pol_width, pol_height);

		#pragma omp section
		filter_machine->doMeanFilterFast1DOMP(&filtered_image, polar_image, 0, pol_width/3, pol_height-1, 2*pol_width/3-1, 'y', 2*m_azi/3, pol_width, pol_height);

		#pragma omp section
		filter_machine->doMeanFilterFast1DOMP(&filtered_image, polar_image, 0, 2*pol_width/3, pol_height-1, pol_width-1, 'y', m_azi, pol_width, pol_height);
	}
/*
	filter_machine->doMedianFilterFast1D(&filtered_image, polar_image, 0, 0, pol_height-1, pol_width/3 -1, 'y', m_azi/3, ring_width, pol_width, pol_height);
	filter_machine->doMedianFilterFast1D(&filtered_image, polar_image, 0, pol_width/3, pol_height-1, 2*pol_width/3 -1, 'y', 2*m_azi/3, ring_width, pol_width, pol_height);
...
int ringRemovalSerial(int argc, char** argv){
	if(!(argc == 15)){
	...
	//	float center_x, center_y, thresh_max = 20, thresh_min = -20, threshold = 1.0;
		...
		for(int img = first_img_num; img < last_img_num + 1; img++){
			...
			image = image_io->readFloatImage(input_path + input_name, &width, &height);    //pixel data is stored in the form image[row][col]
...
//				ring_image = transform_machine->inversePolarTransformBilinear(polar_image, center_x, center_y, pol_width, pol_height, width, height, r_scale, ring_width);
				...
				image_io->writeFloatImage(image, output_path + output_name, width, height);
				clock_t end = clock();
...
double ringRemovalOMP(int argc, char** argv){
	double time = 0;
	if(!(argc == 15)){
		/* arbitrary code for checking errors
		string input_path = ".\\";
...
//			image = tiff_io->read16bitImage(input_path + input_name, &width, &height);    //pixel data is stored in the form image[row][col]
			if(!image){
				...
				if(verbose == 1) printf("Performing Polar Transformation...\n");
				//clock_t start_polar = clock();
				double start_polar = omp_get_wtime();
//				polar_image = transform_machine->polarTransformBilinear(image, center_x, center_y, width, height, &pol_width, &pol_height, thresh_max, thresh_min, r_scale, ang_scale, ring_width);
				polar_image = transform_machine->polarTransformOMP(image, center_x, center_y, width, height, &pol_width, &pol_height, thresh_max, thresh_min, r_scale, ang_scale, ring_width);
				//clock_t end_polar = clock();
				double end_polar = omp_get_wtime();
				//if(verbose == 1) printf("Time for polar Transformation: %f sec\n", (float(end_polar - start_polar)/CLOCKS_PER_SEC));
				if(verbose == 1) printf("Time for polar Transformation: %f sec\n", float(end_polar - start_polar));
				m_azi = ceil(float(pol_height)/(360.0))*angular_min;
				m_rad = 2*ring_width+1;
				//Call Ring Algorithm
				if(img == 7){
				...
				doRingFilterOMP(&polar_image, pol_height, pol_width, threshold, m_rad, m_azi, ring_width, filter_machine, verbose);

				//Translate Ring-Image to Cartesian Coordinates
				if(verbose == 1) printf("Doing inverse polar transform...\n");
				//clock_t start_invpol = clock();
				double start_invpol = omp_get_wtime();
//				ring_image = transform_machine->inversePolarTransformBilinear(polar_image, center_x, center_y, pol_width, pol_height, width, height, r_scale, ring_width);
				ring_image = transform_machine->inversePolarTransformOMP(polar_image, center_x, center_y, pol_width, pol_height, width, height, r_scale, ring_width);
				//clock_t end_invpol = clock();
				double end_invpol = omp_get_wtime();
				//if(verbose == 1) printf("Time for Inverse Polar Transform: %f sec\n", (float(end_invpol - start_invpol)/CLOCKS_PER_SEC));
				if(verbose == 1) printf("Time for Inverse Polar Transform: %f sec\n", float(end_invpol - start_invpol));
...
int main(int argc, char** argv){
	...
	auto start = std::chrono::high_resolution_clock::now();
	ringRemovalSerial(argc, argv);
	auto end = std::chrono::high_resolution_clock::now();
	std::chrono::duration<double> ori_total_time = end - start;

	printf("\n");

	double parTime = ringRemovalOMP(argc, argv);

	printf("\n");

	if(verbose == 1) {
		printf("Serial filtering time: %f sec\n", ori_total_time.count());
		printf("Parallel filtering time: %f sec\n", parTime);
            	printf("Speedup: %.2fx\n", ori_total_time.count() / parTime);
	}
}
