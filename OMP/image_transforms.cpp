...
int ImageTransformClass::findMinDistanceToEdgeOMP(float center_x, float center_y, int width,
											   int height)
{
	int* dist = new int[4];
	dist[0] = center_x+1;
...
float** ImageTransformClass::polarTransformOMP(float** image, float center_x, float center_y, int width,
                                            int height, int* p_pol_width, int* p_pol_height,
                                            float thresh_max, float thresh_min, int r_scale,
                                            int ang_scale, int overhang)
{
	 // Compute minimum distance to edge
    int max_r = findMinDistanceToEdgeOMP(center_x, center_y, width, height);
    int pol_width = r_scale * max_r;
    int pol_height = round(ang_scale * 2.0 * PI * float(max_r));
    *p_pol_width = pol_width;
    *p_pol_height = pol_height;
    ...
    // Allocate memory
    float* image_block = (float*) malloc(pol_height * pol_width * sizeof(float));
    if(image_block == NULL){
        fprintf(stderr, "Memory allocation failed for image_block!\n");
        return NULL;
    }
    float** polar_image = (float**) malloc(pol_height * sizeof(float*));
    if(polar_image == NULL){
        fprintf(stderr, "Memory allocation failed for polar_image pointers!\n");
        free(image_block);
        return NULL;
    }
    polar_image[0] = image_block;
    ...
    float theta_offset = PI / float(pol_height);
    float factor = 2.0 * PI / float(pol_height);
    ...
    for(int row = 0; row < pol_height; row++){
        for(int r = 0; r <= pol_width - r_scale; r++){
            ...
            float theta = row * factor;
            float fl_x = float(r) * cos(theta + theta_offset) / float(r_scale);
            float fl_y = float(r) * sin(theta + theta_offset) / float(r_scale);

            // Round to nearest integer coordinates
            int x = round(fl_x + center_x);
            int y = round(fl_y + center_y);

            // Clamp coordinates to image boundaries
            x = (x < 0) ? 0 : ((x >= width) ? width - 1 : x);
            y = (y < 0) ? 0 : ((y >= height) ? height - 1 : y);

            // Assign pixel value with clamping
...
float** ImageTransformClass::inversePolarTransformOMP(float** polar_image, float center_x,
                                                   float center_y, int pol_width, int  pol_height,
                                                   int width, int height, int r_scale,
                                                   int over_hang)
{
