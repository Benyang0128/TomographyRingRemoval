...
int ImageFilterClass::partition2ArraysOMP(float* median_array, int* position_array, int left, int right, int pivot_index)
{
	float pivot_value = median_array[pivot_index];
	elemSwap(median_array, pivot_index, right);
	elemSwapInteger(position_array, pivot_index, right);
...
void ImageFilterClass::quickSort2ArraysOMP(float* median_array, int* position_array, int left, int right)
{
	if(left < right){
		int pivot_index = int((left + right)/2);
		int new_pivot_index = partition2ArraysOMP(median_array, position_array, left, right, pivot_index);

			quickSort2ArraysOMP(median_array, position_array, left, new_pivot_index - 1);

			quickSort2ArraysOMP(median_array, position_array, new_pivot_index + 1, right);
	}
}
...
void ImageFilterClass::doMedianFilterFast1DOMP(float *** filtered_image, float*** image, int start_row,
										int start_col, int end_row, int end_col, char axis,
										int kernel_rad,	int filter_width, int width, int height)
{
	int row, col;
	float* median_array = (float*) calloc(2*kernel_rad+1, sizeof(float));
	int* position_array = (int*) calloc(2*kernel_rad+1, sizeof(int));
	#pragma omp parallel firstprivate(median_array, position_array)
	{
		if(median_array == NULL || position_array == NULL){
			fprintf(stderr, "Error allocating memory for median filter arrays!");
		}
			...
			#pragma omp parallel
			{
				...
				for(row = start_row; row <= end_row; row++){
					...
					#pragma omp parallel
					{
						...
						quickSort2ArraysOMP(median_array, position_array, 0, 2*kernel_rad);
					}
					filtered_image[0][row][col] = median_array[kernel_rad];
 ...
 */
void ImageFilterClass::doMeanFilterFast1DOMP(float*** filtered_image, float*** image,
 									      int start_row, int start_col, int end_row, int end_col,
										  char axis, int kernel_rad, int width, int height)
{
	float mean = 0, sum = 0, previous_sum = 0, num_elems = float(2*kernel_rad + 1);
