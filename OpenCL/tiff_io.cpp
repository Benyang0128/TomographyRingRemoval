...
float** TiffIO::readFloatImage(string image_name, int* w_ptr, int* h_ptr) {
    ...
    TIFF* tif = TIFFOpen(image_name.c_str(), "r");  // Declare and initialize tif
    ...
    *h_ptr = height;
    ...
    tsize_t* bc;
    tsize_t stripsize;
    uint16_t num_strips = TIFFNumberOfStrips(tif);  // Use uint16_t
    TIFFGetField(tif, TIFFTAG_STRIPBYTECOUNTS, &bc);
    stripsize = bc[0];
    buffer = (float*)_TIFFmalloc(stripsize);
    uint32_t location = 0;  // Use uint32_t
...
void TiffIO::writeFloatImage(float** image_rows, string output_name, int width, int height) {
    ...
    for (uint32_t row = 0; row < height; row++) {  // Use uint32_t
